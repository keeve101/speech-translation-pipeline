...
# https://github.com/snakers4/silero-vad/blob/9060f664f20eabb66328e4002a41479ff288f14c/utils_vad.py#L340
# (except changed defaults and modified to remove torch)
# Their licence is MIT
    ...
    def __call__(self, x, return_seconds=False):
        ...
        if not isinstance(x, np.ndarray):
            try:
                ...
                raise TypeError("Audio cannot be converted to numpy array. Convert it manually")
...
class OnnxWrapper():
    ...
    def _validate_input(self, x, sr: int):
        if x.ndim == 1:
            x = x[np.newaxis, :]  # equivalent to unsqueeze(0)
        if x.ndim > 2:
    ...
    def __call__(self, x, sr: int):
        ...
        if not len(self._context):
            self._context = np.zeros((batch_size, context_size), np.float32)
    ...
    def audio_forward(self, x, sr: int):
        ...
        if x.shape[1] % num_samples:
            ...
            # Replace torch padding with numpy padding
        ...
        # Remove .cpu() as it's not needed for numpy arrays
        return stacked
...
def get_speech_timestamps(audio: np.ndarray,
                          model,
                          threshold: float = 0.5,
                          sampling_rate: int = 16000,
    ...
    Parameters
    ----------
    audio: np.ndarray, one dimensional
    ...
    for current_start_sample in range(0, audio_length_samples, window_size_samples):
        ...
        if len(chunk) < window_size_samples:
            # Replace torch pad with numpy pad
            pad_size = int(window_size_samples - len(chunk))
            chunk = np.pad(chunk, (0, pad_size), mode='constant', constant_values=0)
